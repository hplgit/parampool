# Just copy from flask - needs edit

import os

def generate_model_pool(compute_function, parser, outfile, pool):
    """
    Generate Argparser object (parser) form by iterating through
    ...
    def leaf_func(tree_path, level, item, user_data):
        # 2DO: Extract data from item and make code for calls to parser.add
        # ...
...
def generate_model_inspect(compute_function, parser, outfile, default_field):
    """
    Generate the appropriate Argparser definitions
    by using inspect on the given function.
    Default values in the function are kept as default
    values in the command-line options.
    Variables without a default value are assumed
    to be represented by `default_field` type.
    """
    # 2DO: generate Argparser code for each arg in compute_function
...
def generate_model(compute_func, classname, outfile, default_field,
    ...
    if pool is not None:
        return generate_model_pool(
            compute_func, classname, outfile, pool)
    else:
